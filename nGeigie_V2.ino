...
Hardware runs on a Teensy 3.1



 by Allan Lind: alind@joho.com
 Code from Lionel, akiba, allanlind and Kalin of Safecast
...
ß
	...
	// Print a message to the LCD.
		  ...
		  lcd.print(VERSION);
                  lcd.setCursor(0, 3);
                  lcd.print("       Safecast 2015");
                  delay(3000);
                  
    //enable ethenet high is disabled low is enabled
       pinMode(16, OUTPUT);
       digitalWrite(16, LOW);
...
//        lcd.print ("loading setup");
  ...
  //Check if Time is setup if setup use as name for logfile
...
//       lcd.print(config.gw2);
    ...
    //3G enabled check
        if (config.g3_enabled) {
          delay(3000);
          lcd.clear();
          lcd.setCursor(0, 0);
          lcd.print("3G card enabled");
          Serial.println("3G card enabled");
          } else {
            delay(3000);
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("3G card disabled");
            Serial.println("3G card disabled");
